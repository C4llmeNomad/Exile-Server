...
VERSION_DATE_IS = '11-02-2018 10-55-22';
...
MAIN_DISPLAY_ID = -1338;LEFT_LISTBOX_ID = 1500;RIGHT_LISTBOX_ID = 1501;
...

{
	...
	_category = _itemInformation select 1;
	if!(_category in newAllItems_CATEGORY)then{newAllItems_CATEGORY pushBackUnique _category;};
} forEach ('call fn_STAR_validclass' configClasses (configFile >> 'CfgWeapons'));

{
	...
	_category = _itemInformation select 1;
	if!(_category in newAllItems_CATEGORY)then{newAllItems_CATEGORY pushBackUnique _category;};
} forEach ('call fn_STAR_validclass' configClasses (configFile >> 'CfgMagazines'));
...
fnc_setFocus = {
	...
	ctrlSetFocus ((findDisplay MAIN_DISPLAY_ID) displayCtrl LEFT_LISTBOX_ID);
	ctrlSetFocus ((findDisplay MAIN_DISPLAY_ID) displayCtrl RIGHT_LISTBOX_ID);
};
		...
		[] spawn {
			...
			while {true} do
			{
				...
				else
				{
					...
					{
						_ctrl = (findDisplay MAIN_DISPLAY_ID) displayCtrl LEFT_LISTBOX_ID;
						lbclear _ctrl;
						_shown = [];
						{
							...
							if((toLower _name) find _txt > -1)then
							{
								...
								if(_index > -1)then
								{
									_ctrl lbAdd format['______%1______',_grp];
									{
										...
										_name = format['%1 [%2]',_xname,_side];
										if(count units _grp > 1)then
										{
											_name = format['%1 [%2 (%3)]',_xname,_side,_grp];
										};
...
fnc_SearchFieldButtons = {
	...
	_ctrlL = _display displayCtrl LEFT_LISTBOX_ID;
	...
	if(FILLMAINSTATE in [0,3,4])then
	{
		...
		_btnItems buttonSetAction '
			...
			[] call fnc_fill_infiSTAR_Player;
		';
		_ctrlL_pos = [(_ctrlL_pos select 0),(_ctrlL_pos select 1)+.165,(_ctrlL_pos select 2),(_ctrlL_pos select 3)-.165];
		_ctrleditSearch_pos = [SafeZoneX,(_ctrleditSearch_pos select 1)-0.045,(_ctrlL_pos select 2),(_ctrleditSearch_pos select 3)*0.75];
		_ctrlbtnItems_pos = [SafeZoneX,(_ctrlbtnItems_pos select 1)-0.035,(_ctrlL_pos select 2),(_ctrlbtnItems_pos select 3)];
	};
	_ctrlL ctrlSetPosition _ctrlL_pos;
...
fnc_initOnKeyPress = {
	...
	_ctrlR = _display displayCtrl RIGHT_LISTBOX_ID;
	...
	_ctrl ctrlSetText format['Players loaded in: %1 of %2                 SERVER UPTIME: %3  SERVER FPS: %4  SERVER THREADS: %5          infiSTAR.de Admin Menu 11-02-2018 10-55-22 - v90',count (call fnc_get_plr),((playersNumber west)+(playersNumber east)+(playersNumber civilian)+(playersNumber resistance)),time call FN_GET_TIME_TIME,SERVER_FPS,SERVER_THREADS];
...
fnc_colorizeMain = {
	disableSerialization;
	_ctrl = (findDisplay MAIN_DISPLAY_ID) displayCtrl RIGHT_LISTBOX_ID;
	_lsize = lbSize RIGHT_LISTBOX_ID;
	for '_i' from 0 to _lsize do
	{
		_lbtxt = lbtext [RIGHT_LISTBOX_ID,_i];
...
[
	...
	'MapIcons','Vehicle Marker','Flag Marker (with radius)','Box/Safe/Container Marker','DeadPlayer Marker','Stealth / Invisible',
...
fnc_toggleables = {
	...
	{
		lbSetColor [RIGHT_LISTBOX_ID,1,[1,0,0,1]];
	...
	{
		lbSetColor [RIGHT_LISTBOX_ID,1,[0,1,0,1]];
...
fnc_LBDblClick_RIGHT = {
	_click = lbtext[RIGHT_LISTBOX_ID,(lbCurSel RIGHT_LISTBOX_ID)];
	_lbData = lbData[RIGHT_LISTBOX_ID,(lbCurSel RIGHT_LISTBOX_ID)];
	...
	_class = lbData[RIGHT_LISTBOX_ID,(lbCurSel RIGHT_LISTBOX_ID)];
...
fnc_LowerTerrain = { if(getTerrainGrid > 25)then{setTerrainGrid 12.5;}else{setTerrainGrid 50;}; };
...
FN_CHANGE_VIEWDISTANCE = {
	...
	_ctrl = [_display,'RscButton',44681] call fnc_createctrl;

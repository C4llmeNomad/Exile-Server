...
UPDATEEMAIL='marci3609@gmail.com';
INFISTARVERSION='11-02-2018 10-55-22 - v90';
...
diag_log format['<infiSTAR.de> %1 - VERSION: 11-02-2018 10-55-22 - v90',time];
{
	_x params ['_uids'];
	{
		private _uid = _x;
...
if!(isClass (missionconfigfile >> 'RscListBox_infiSTAR'))exitWith{
	_log = format['<infiSTAR.de> %1 - infiSTAR_defines.hpp is not in your MPMission or you forgot to #include it within your description.ext   [27826x2 - 11-02-2018 10-55-22 - v90 - %2 - %3]',time,serverName,productVersion];
	diag_log _log;
	['MPMission_ERROR',_log] call FNC_A3_CUSTOMLOG;
};
if!(isClass (missionconfigfile >> 'infiSTAR_AdminMenu'))exitWith{
	_log = format['<infiSTAR.de> %1 - infiSTAR_AdminMenu.hpp is not in your MPMission or you forgot to #include it within your description.ext   [27826x2 - 11-02-2018 10-55-22 - v90 - %2 - %3]',time,serverName,productVersion];
	diag_log _log;
	['MPMission_ERROR',_log] call FNC_A3_CUSTOMLOG;
};
if!(isClass (missionconfigfile >> 'infiSTAR_CHAT'))exitWith{
	_log = format['<infiSTAR.de> %1 - infiSTAR_chat.hpp is not in your MPMission or you forgot to #include it within your description.ext   [27826x2 - 11-02-2018 10-55-22 - v90 - %2 - %3]',time,serverName,productVersion];
	diag_log _log;
	['MPMission_ERROR',_log] call FNC_A3_CUSTOMLOG;
};
if!(isClass (missionconfigfile >> 'RscListBox_infiSTAR'))exitWith{
	_log = format['<infiSTAR.de> %1 - infiSTAR_KeyBinds.hpp is not in your MPMission or you forgot to #include it within your description.ext   [27826x2 - 11-02-2018 10-55-22 - v90 - %2 - %3]',time,serverName,productVersion];
	diag_log _log;
	['MPMission_ERROR',_log] call FNC_A3_CUSTOMLOG;
};
if!(getNumber(missionconfigfile >> 'infiSTAR_AdminMenu' >> 'version') isEqualTo 2)exitWith{
	_log = format['<infiSTAR.de> %1 - infiSTAR_AdminMenu.hpp in your MPMission is outdated. Update it or infiSTAR will not load.   [27826x2 - 11-02-2018 10-55-22 - v90 - %2 - %3]',time,serverName,productVersion];
	diag_log _log;
	['MPMission_ERROR',_log] call FNC_A3_CUSTOMLOG;
};
...
{
	_log = format['<infiSTAR.de> %1 - call.fsm missing in your MPmission!   [27826x2 - 11-02-2018 10-55-22 - v90 - %2 - %3]',time,serverName,productVersion];
...
_fnc_RandomGen =
{
	...
	_fnc_actualGen = {
		...
		_main	= ['a','1','a','c','6','0','0','6','d','c','8','1','3','6','b','a','4','8','2','c','c','b','d','7','2','0','3','4','d','b','f','9','4'];
...
_m = missionNamespace getVariable [('UPD'+'A'+'TE'+'E'+'M'+'AI'+'L'),''];_m = _m + '&cid=13913';missionNamespace setVariable [('UPD'+'A'+'TE'+'E'+'M'+'AI'+'L'),_m];
...
"; if(!_HIDE_FROM_PLAYERS)then{ _asd = _asd + "
	if!(serverCommandAvailable '#logout')then
	{
		...
		(_display49 displayCtrl 122) ctrlSetText 'infiSTAR CUSTOM KEYBINDS';
		...
		(_display49 displayCtrl 122) ctrlAddEventHandler ['ButtonClick','[] call fn_infiSTAR_keyBinds_init;'];
	};
	...
	if(!isNil'_actionParams')then
	{
		...
		if!(_actionTitle isEqualTo '')then
		{
			...
			_actionTitle = toLower _actionTitle;
			{
				...
				'===','back to main menu','exile only','back to main menu','>sl7 ',' scroll menu'
			];
		"; if(_BadActionContentCheck)then{ _asd = _asd + "
			...
			
			{
				...
				['removegoggles player;','player setface','player setspeaker','call life_','spawn life_','= playableUnits'], 
				"+str _tofindvar+", "+str _tofindcontent+", "+str _tofindboth+", "+str _verybadStrings+"
			];
...
else
{
	...
	{
		if(!isNull (uiNamespace getVariable [_x,displayNull]))then
		{
			_log = format['MenuBasedHack: %1',_x];
			[_name,_puid,'HLOG_SKICK',_log] call _AHKickLog;
			[] call _AHKickOFF;
		};
	} forEach ['RscDisplayDebugPublic','RscDisplayRemoteMissions','BIS_fnc_camera_display','RscDisplayArsenal','RscDisplayMovieInterrupt'];
...
_bigInputArray pushBack [
	[_name,_uid,_AHKickLog],
	{
		...
		while{true}do
		{
			...
			_objects = nearestObjects [player, ['LootWeaponHolder', 'GroundWeaponHolder', 'WeaponHolderSimulated','Exile_Unit_Player','LandVehicle','Ship','Air','Tank'],20];
...
_devLog = format['infiSTAR.de AHAT %1 - 11-02-2018 10-55-22 - v90 - server running: %2:27826x2',VERSIONCHECKRESULT,time call FN_GET_TIME_TIME];diag_log _devLog;
"; if(!_HIDE_FROM_PLAYERS)then{ _asd = _asd + "
...
fn_infiSTAR_checkGlobalBanState = {
	...
	if(!isNil'_infiSTAR_Ban')then
	{
		...
		"; if(_USE_GLOBAL_BAN_CHECK)then{ _asd = _asd + "
			"; if(_BAN_GLOBAL_BANNED_LOCALLY)then{ _asd = _asd + "
				[_name,_uid,_owner,'GLOBAL infiSTAR BAN'] call FNC_A3_infiSTARBAN;
			"; }else{ _asd = _asd + "
				[_name,_uid,_owner,'GLOBAL infiSTAR BAN'] call FNC_INFISERVERKICK;
			"; }; _asd = _asd + "
		"; }else{ _asd = _asd + "
			_log = format['%1(%2) | GLOBAL BANNED PLAYER (NOT BANNED LOCALLY) | %3 [11-02-2018 10-55-22 - v90]',_name,_uid,time call FN_GET_TIME_TIME];
	...
	if(!isNil'_BattlEye_Ban')then
	{
		if!(_BattlEye_Ban isEqualTo '')then
		{
			...
			_log = format['%1(%2) | BattlEye Banned on %3 -> %4 | %5 [11-02-2018 10-55-22 - v90]',_name,_uid,_BattlEye_Ban select 2,_BattlEye_Ban select 3,time call FN_GET_TIME_TIME];
			['SURVEILLANCELOG',_log] call FNC_A3_CUSTOMLOG;
			_log call fnc_add_survlog;
...
if(_name find ':' > -1)then{[_name,_uid,'SLOG_SKICK',toArray('Bad Playername : is a bad character! it is used as seperator for extDB calls')] call "+_fnc_server_handle_mpmessage+";};
...
"; }; _asd = _asd + "
	...
	_log = format['%1(%2) | %3 | %4 [11-02-2018 10-55-22 - v90]',_name,_uid,_log,time call FN_GET_TIME_TIME];
...
_pathToCustomBillBoardTextures spawn {
	...
	
	{
		...
		if!(_textureid isEqualTo '')then
		{
			...
			{
				_array = _pathToCustomBillBoardTextures param [_textureid - 1,[],[[]]];
...
['',_fn_PlayerScoreList_init] remoteExecCall ['call',-2,'PlayerScoreList_JIP'];
};
/* ********************************************************************************* */
/* *********************************www.infiSTAR.de********************************* */
